...
export class AuthenticationService {
  ...
  login(username: string, password: string) {
    return this.http.post<any>(`http://pathfinder.tk:8080/login`, { username, password }, { observe: 'response' })
      .pipe(first())
      .subscribe(user => {
  ...
  register(username: string, password: string) {
    ...
    return this.http.post<any>('http://pathfinder.tk:8080/user/sign-up', { username, password })
      .pipe(map(user => {
